...
데이터 소스:
  F1/F3/F4A/F4B/F4C/F6 : data_unified/ PKL (전처리 前 원본) 병렬 스캔
  F5                    : 2_preprocess/outputs/cleaning_report.csv +
                          data_unified/ PKL (용량 시계열)
...
def plot_f5(window: int, sigma: float, min_std: float, top_n: int):
    ...
    for ai, (_, row) in enumerate(rm_df.iterrows()):
        ...
        pkl = PROJECT_ROOT / "data_unified" / ds / f"{cell_id}.pkl"
        if not pkl.exists():
...
def main():
    ...
    if not pkl_list:
        print("data_unified/ 에 PKL 파일 없음. 경로 확인 필요."); return
