...
필터 적용 순서:
  ...
  [필터5] _remove_outlier_cycles      — Rolling Median 이상 사이클 제거 (window=11, sigma=2.5)
...
def _remove_outlier_cycles(df: pd.DataFrame,
    ...
    """방전 용량 시계열에서 Rolling Median 기반 이상 사이클 제거.
...
def _preprocess_worker(args) -> tuple:
    ...
    (src_path_str, dst_path_str,
     ...
     chg_seg_gap_s, chg_seg_gap_factor) = args
    ...
    try:
        ...
        n_cycles_removed = n_empty + n_dt_dis + n_rolling + n_vend
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="unified PKL → 6단계 이상 사이클·행 제거 → postprocess 저장"
    )
        ...
        if not mit_src.exists():
            ...
            all_records.extend(
                process_dir(mit_src, mit_dst, w, s, m, vm,
                            dgs, dgf, cgs, cgf, csgs, csgf, args.workers))
...
def _plot_preprocess_samples(out_dir: Path,
    ...
    for ds, (cell_id, cyc_b, cap_b) in items:
        ...
        for col in ["n_removed_empty", "n_removed_dt_dis",
                    "n_removed_rolling", "n_removed_vend"]:
