...
costs no recomputation. This module is self-contained; hi_correlation.py's segment
analysis imports ``_seg_stat/_seg_diff/_seg_lfp`` adapters from here — single source.
...
def _dtw_distance(a, b):
    ...
    for i in range(1, n):
        ...
        for j in range(j_lo, j_hi):
            best = dtw[i - 1, j]
            if dtw[i, j - 1]     < best: best = dtw[i, j - 1]
            if dtw[i - 1, j - 1] < best: best = dtw[i - 1, j - 1]
            dtw[i, j] = d[i, j] + best
...
class W:
    ...
    @cached_property
    def ai(self):
        return np.abs(self.i)

    @cached_property
    def dt(self):
        return np.clip(np.diff(self.t, prepend=self.t[0]), 0.0, None)

    @cached_property
    def q(self):
        return np.cumsum(self.ai * self.dt) / 3600.0
    ...
    @cached_property
    def t_norm(self):
        ts = np.zeros(len(self.dt))
        ts[1:] = np.cumsum(self.dt[1:])
        tt = float(ts[-1])
    ...
    def from_segment(cls, vs, ims, dts, qcs):
        ...
        injects the provided |i|, dt, cumulative-q directly — bypasses cached_property."""
        w = cls.__new__(cls)
        w.v = np.asarray(vs, float)
        w.i = None
        w.t = None
...
@hi
def stat_v_mean_cw(w):
    """S01 전하 가중 평균 전압 Σ(V·|I|·dt)/Σ(|I|·dt)."""
...
def stat_v_ent(w):
    if len(w.v) < 5: return np.nan
    cnt = np.histogram(w.v, bins=20)[0].astype(float)
    tot = cnt.sum()
...
def _dqdv_valley(w):
    """D16–D17: ICA valley (피크 대비 ≤20%). (h, v) 또는 (nan, nan)."""
    vmids, dqdv_sm = w.ica
...
def diff_dqdv_valley_h(w):
    """D16 ICA valley height. 밸리 미발견 시 0.0 폴백."""
    h, _ = _dqdv_valley(w)
    return h if not np.isnan(h) else 0.0

@hi
def diff_dqdv_valley_v(w):
    """D17 ICA valley voltage. 밸리 미발견 시 0.0 폴백."""
    _, v = _dqdv_valley(w)
...
@hi
def lfp_plateau_dvdq_std(w):
    """L04 플래토 구간 dV/dQ 표준편차 (히스테리시스·불균일도 지표)."""
    p = _plateau(w)
    if p is None: return np.nan
    qm, v_sm, dvdq_sm, q_tot, fin_b, plt_mask, dq_b, n_b = p
    if plt_mask.sum() < max(2, int(0.05 * n_b)): return np.nan
    ...
    return float(np.std(plt_dv[fin_pd])) if fin_pd.sum() >= 3 else np.nan
...
@hi
def lfp_v_dev_mid(w):
    """L06 V(q_mid) - V_linear(q_mid) 중간점 전압 편차."""
    p = _plateau(w)
    if p is None: return np.nan
    qm, v_sm, dvdq_sm, q_tot, fin_b, plt_mask, dq_b, n_b = p
...
@hi
def lfp_vq_slope_mid(w):
    """L09 dV/dQ at q_mid (측정 전압 기반, OCV 아님)."""
    p = _plateau(w)
    if p is None: return np.nan
    qm, v_sm, dvdq_sm, q_tot, fin_b, plt_mask, dq_b, n_b = p
    if not fin_b.any(): return np.nan
...
def _inflect(w):
    """L10–L11: V-Q 변곡점. (inflect_v, inflect_q_frac) 또는 (nan, nan)."""
    p = _plateau(w)
...
def lfp_dv_dt_plateau(w):
    ...
    q_lo = float(qm[plt_mask][0]) - dq_b / 2
    q_hi = float(qm[plt_mask][-1]) + dq_b / 2
...
def _seg_group(prefix: str, vs, ims, dts, qcs, seg: str) -> dict:
...
def _seg_stat(vs, ims, dts, qcs, seg):
    return _seg_group("stat_", vs, ims, dts, qcs, seg)

def _seg_diff(vs, ims, dts, qcs, seg):
    return _seg_group("diff_", vs, ims, dts, qcs, seg)

def _seg_lfp(vs, ims, dts, qcs, seg):
    return _seg_group("lfp_",  vs, ims, dts, qcs, seg)
...
if __name__ == "__main__":
    ...
    if _pkl:
        ...
        print(f"등록된 HI: {len(hi_names())}개 (@hi {len(_HI)} + morph {len(_MORPH_NAMES)})")
    else:
        print(f"등록된 HI: {len(hi_names())}개 (@hi {len(_HI)} + morph {len(_MORPH_NAMES)})")
        print("HI 목록:")
        for n in hi_names():
