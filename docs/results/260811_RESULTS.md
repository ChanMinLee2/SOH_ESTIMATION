...
## 12. 실행 결과 — MLP_S 3-seed 재현성 검증 (k=25) (2026-08-13, 2026-08-14 정정)

§10/§11과 같은 설계로 `scen_k=25`에서 반복했다. **최초 버전(2026-08-13)은 `q_frac_ref`
쪽이 `$env:SOH_EXCLUDE_STAT_LEAK="1"` 없이 실행돼(`N_HI=66` vs `random`의 64) HI 개수가
어긋난 비교였다 — `SOH_EXCLUDE_STAT_LEAK=1`을 맞춰 재실행한 결과로 아래 표를
교체했다(2026-08-14).** 이제 두 축 다 `N_HI=64`로 §10/§11과 동일 조건이다.

| axis | seed | run 디렉터리 | N_HI | R²(oracle) | RMSE | MAE | MAPE |
|---|---|---|---|---|---|---|---|
| `random` | 42 | `0813_0209_p2_mlp_rand_noleak` | 64 | 0.9361 | 1.760%p | 1.301%p | 1.436% |
| `random` | 0 | `0813_0228_p2_mlp_rand_noleak` | 64 | 0.9340 | 1.721%p | 1.263%p | 1.389% |
| `random` | 123 | `0813_0249_p2_mlp_rand_noleak` | 64 | 0.9432 | 1.678%p | 1.255%p | 1.383% |
| **`random` 평균±표준편차** | | | | **0.9378 ± 0.0048** | **1.720 ± 0.041%p** | **1.273 ± 0.025%p** | **1.403 ± 0.029%** |
| `q_frac_ref` | 42 | `0813_1835_p2_mlp_q_fr_35%_20%_noleak` | 64 | 0.9548 | 1.481%p | 1.076%p | 1.189% |
| `q_frac_ref` | 0 | `0813_1951_p2_mlp_q_fr_35%_20%_noleak` | 64 | 0.9499 | 1.500%p | 1.080%p | 1.185% |
| `q_frac_ref` | 123 | `0813_2108_p2_mlp_q_fr_35%_20%_noleak` | 64 | 0.9583 | 1.438%p | 1.033%p | 1.140% |
| **`q_frac_ref` 평균±표준편차** | | | | **0.9543 ± 0.0042** | **1.473 ± 0.032%p** | **1.063 ± 0.026%p** | **1.171 ± 0.027%** |

통계: 격차 ΔR²=+0.0165, pooled std=0.0045, **Cohen's d=3.65**, Welch t=4.47,
**p=0.0115**(§10/§11과 동일 방식, `scipy.stats.ttest_ind(equal_var=False)`).

### 해석 — 대칭 비교로 바로잡으니 "단조 증가"가 아니라 k=15에서 정점을 찍는 모양이 된다

정정 전(HI 개수 어긋난 버전)에서는 k=5→15→25가 +0.0146→+0.0180→+0.0197로 계속
커지는 것처럼 보였는데, **HI 개수를 맞춰 다시 보니 순서가 +0.0146 → +0.0180 →
+0.0165로, k=15에서 정점을 찍고 k=25에서 살짝 내려온다.** "단조 증가" 서술은
철회한다 — 지금 3개 지점만으로는 뚜렷한 추세(단조 증가/감소/정점형)를 확정하기
어렵고, k=15가 우연히 높게 나온 것인지 진짜 정점인지는 k=25/65가 더 쌓이거나
seed를 늘려야 판단할 수 있다.

다만 **`q_frac_ref` 우위 자체는 이번에도 통계적으로 유의하다**(d=3.65, p=0.012,
두 축의 3-seed 구간이 여전히 안 겹침: `q_frac_ref` 0.9499~0.9583 vs `random`
0.9340~0.9432). `avg_computed_his`도 `q_frac_ref`가 29.0~29.7, `random`이
25.0~25.5로, 이번엔 `N_HI`가 동일(64)한데도 `q_frac_ref` 쪽이 여전히 더 많은
HI를 union으로 쓴다 — §10에서 이미 확인한 "시나리오 수가 많을수록(6 vs 2) 게이트
union이 커진다"는 설명과 일치하는 결과라, HI 개수 불일치 없이도 재현되는
패턴임을 확인했다.

**요약(k=5/15/25, 전부 HI 개수 대칭)**: ΔR² = +0.0146 / +0.0180 / +0.0165 —
전 구간에서 `q_frac_ref`가 통계적으로 유의하게 우위이지만, k에 따른 격차 크기
자체의 추세는 아직 뚜렷하지 않다.

---

## 13. 실행 결과 — MLP_S 3-seed 재현성 검증 (k=65) (2026-08-14)

§10~§12와 같은 설계로 `scen_k=65`에서 반복했다. 두 축 다 `exclude_stat_leak: true`
(`N_HI=64`)로 확인됨 — HI 개수는 대칭이다.

| axis | seed | run 디렉터리 | R²(oracle) | RMSE | MAE | MAPE |
|---|---|---|---|---|---|---|
| `random` | 42 | `0814_0129_p2_mlp_rand_noleak` | 0.9329 | 1.805%p | 1.311%p | 1.447% |
| `random` | 0 | `0814_0152_p2_mlp_rand_noleak` | 0.9180 | 1.918%p | 1.346%p | 1.480% |
| `random` | 123 | `0814_0216_p2_mlp_rand_noleak` | 0.9412 | 1.707%p | 1.269%p | 1.398% |
| **`random` 평균±표준편차** | | | **0.9307 ± 0.0117** | **1.810 ± 0.106%p** | **1.308 ± 0.039%p** | **1.441 ± 0.041%** |
| `q_frac_ref` | 42 | `0814_0312_p2_mlp_q_fr_35%_20%_noleak` | 0.9561 | 1.459%p | 1.055%p | 1.165% |
| `q_frac_ref` | 0 | `0814_0447_p2_mlp_q_fr_35%_20%_noleak` | 0.9468 | 1.546%p | 1.131%p | 1.235% |
| `q_frac_ref` | 123 | `0814_0613_p2_mlp_q_fr_35%_20%_noleak` | 0.9569 | 1.461%p | 1.079%p | 1.196% |
| **`q_frac_ref` 평균±표준편차** | | | **0.9533 ± 0.0056** | **1.489 ± 0.049%p** | **1.088 ± 0.039%p** | **1.199 ± 0.035%** |

(둘 다 `avg_computed_his=64.00`로 정확히 일치 — `N_HI=64`에서 `scen_k=65`가
no-op라는 기존 확인과 일치.)

통계: 격차 ΔR²=+0.0226(지금까지 k=5/15/25보다 가장 큼), pooled std=0.0092,
Cohen's d=2.45, Welch t=3.00, **p=0.0607**.

### 해석 — 점 추정 격차는 가장 크지만, `random`의 분산이 커져서 통상 기준(α=0.05)엔 못 미친다

k=65에서 **`random`의 표준편차(0.0117)가 k=5/15/25 때(0.0043~0.0049)의 2.5~3배로
뛰었다** — 3개 seed 중 하나(seed=0, R²=0.9180)가 유독 낮게 나왔다. `q_frac_ref`는
반대로 std=0.0056로 k=25(0.0042)와 비슷한 수준을 유지했다. 그 결과 점 추정 격차
(+0.0226)는 지금까지 중 가장 크지만, 분모(pooled std)도 같이 커져서 Welch
t-test의 p값이 **0.061로 관례적 유의수준(0.05)을 살짝 넘긴다** — "유의하지 않다"고
단정할 정도는 아니지만(p<0.1 수준의 약한 신호), §10~§12처럼 "노이즈로 설명 안
된다"고 자신 있게 말할 수는 없다.

**왜 `random`만 k=65에서 불안정해졌는지**는 §8(full_cycle 스윕)에서 이미 나온
패턴과 같은 방향이다 — `scen_k=65`는 `N_HI=64`에서 no-op(전체 HI 사용, 스파스화
없음)이라, §8이 "scen_k=65(전체 HI 사용)가 오히려 가장 나쁘고 불안정했다"고
관찰한 것과 정확히 같은 조건이다. `random`은 시나리오가 2개뿐이라 게이트가
훈련 신호를 덜 받는데(§10의 "시나리오 수가 적을수록 게이트가 더 균일하게
수렴" 관찰과 연결), 여기에 스파스화 압력까지 없어지니(k=65) seed에 따른
변동이 더 커진 것으로 보인다. `q_frac_ref`는 시나리오가 6개라 게이트 수 자체가
많아 이 불안정성의 영향을 상대적으로 덜 받는 것일 수 있다 — 확정은 아니다.

**k=5/15/25/65 전체 요약**: ΔR² = +0.0146 / +0.0180 / +0.0165 / +0.0226(p=0.061,
유일하게 통상 유의수준 미달). `q_frac_ref` 우위 방향 자체는 4개 k 전부 일관되지만,
유의성의 "확실함" 정도는 k=65에서 약해진다 — seed를 늘리거나 `random`의 k=65
불안정성 원인을 더 파보는 게 다음 단계로 적절해 보인다.
