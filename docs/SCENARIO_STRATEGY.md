...
## 12. 신규 옵션 설계(2026-08-02) — `zone_invasion` / `random_length`

### 12.0 배경 및 동기

이전 대화(같은 세션)에서 "세그먼트 분할의 최우선 목적은 현실과 유사한 환경 조성"이라는
관점으로 다시 짚었을 때, 두 가지 현재 구조의 한계가 드러났다:

1. **모든 축·모든 모드(고정격자든 `random_segment`든)가 세그먼트를 존 경계 안에
   가둔다.** 실제 배포에서 BMS가 관측하는 부분충전 세션은 존 경계(plateau
   진입/이탈, q_frac 임계값 등)와 무관하게 임의 시점에 시작·종료된다 — 세그먼트가
   존 하나에 완전히 포함된다는 가정 자체가 학습-배포 분포 불일치를 만들 수 있다.
   `docs/REGRESSION_UPGRADE.md` 세션에서 확인한 2026년 논문 3편(clustered voltage
   segments, multistage partial charging, iMOE)도 전부 "관측 창은 임의 시작점에서
   시작"을 전제하거나(iMOE는 이를 셀링포인트로 명시) 실사용 행동으로 정당화된 넓은
   창을 쓰고, 그 안의 이질성은 라우터/클러스터로 흡수한다 — 지금 구조는 그 흐름과
   반대로 "세그먼트=존"을 강제하고 있다.
2. **모든 축이 존당 `n_samples`개의 고정/제한된 위치만 샘플링한다.** 존 전체
   범위 중 실제로 세그먼트 내용으로 쓰이는 부분은 일부뿐이고(고정격자는
   `n_samples`개의 결정론적 위치, `random_segment`는 무작위지만 여전히
   `seg_len_pts` 창 크기·개수 제한), 존의 나머지 구간은 한 번도 세그먼트 내용으로
   등장하지 않을 수 있다. `q_abs`의 `adaptive_samples`가 존 폭에 맞춰 개수를
   조정하는 것도 이 문제의 부분적 대응이었지만 **위치**(zone 전체 커버리지)까지
   보장하진 않는다.

이 두 가지에 대응하는 옵션을 설계한다. **둘 다 기본값은 `False`(기존 동작 완전
보존)이고, 서로 독립적으로 켤 수 있다** — `random_segment`/`adaptive_samples`처럼
opt-in이며 기존 실험·저장 경로에 영향을 주지 않는다.

### 12.1 `zone_invasion` — 존 경계 침범 허용

**목적**: 세그먼트가 자신이 "속한"(라벨을 결정하는) 존을 넘어 인접 존까지 내용이
이어지는 것을 허용해, 실제 관측 세션이 존 경계와 무관하게 걸릴 수 있다는 현실을
반영한다.

**현재(기본값 `zone_invasion=False`) 동작 — 3개 축 공통 패턴**:
- 고정격자 모드: `_start_positions(zone_start, zone_end)`가 유효 시작 범위를
  `[zone_start, zone_end - seg_len]`으로 제한 → `start + seg_len ≤ zone_end`가
  구조적으로 보장됨(경계 침범 원천 불가).
- `random_segment` 모드: `sample_random_windows`에 넘기는 `zone_idx` 자체가 이미
  존 마스크(`(q>=lo_q)&(q<hi_q)`)로 필터링된 인덱스라, 그 안에서 뽑는 어떤 창도
  존 밖으로 못 나간다.

**`zone_invasion=True` 의미론(제안)**:
- 고정격자 모드: `_start_positions`의 유효 시작 범위 상한을 `zone_end - seg_len`
  대신 `zone_end`로 완화 — 시작점은 여전히 존 안이어야 하지만(라벨 결정 기준
  유지), `start + seg_len`이 `zone_end`를 넘어 다음 존까지 내용이 이어질 수 있다.
  단, **사이클 자체의 관측 가능 범위(1.0 또는 그 방향의 실측 `q[-1]`)는 여전히
  절대 상한**이다 — zone_invasion은 "존 간" 경계만 허무는 것이지 "관측된 데이터가
  없는 곳"까지 만들어내진 않는다.
- `random_segment` 모드: 현재 `zone_idx`(시작점 후보 = 콘텐츠 후보, 동일 배열)를
  "시작점 후보 풀"과 "콘텐츠 추출 풀"로 분리해야 한다 — 시작점은 존 마스크로 제한한
  인덱스 집합에서 뽑되(`sample_random_windows`의 `s`), 실제 `seg_len_pts`개를
  잘라내는 대상 배열은 그 존 마스크가 아니라 **해당 방향 전체(또는 최소한 다음
  존까지 확장한) 인덱스 배열**이어야 한다. `sample_random_windows` 시그니처에
  "시작점 탐색 범위"와 "슬라이스 원본 배열"을 분리하는 파라미터 추가가 필요(현재는
  둘이 같은 `zone_idx`) — 예: `sample_random_windows(start_pool, content_pool,
  seg_len_pts, n_samples, min_pts, rng)`.
- **라벨(scenario_id/latent_class) 규칙**: 시작점이 속한 존으로 결정(기존 구조
  최소 변경, 그리고 "이 관측을 시작한 시점에 이 시나리오였다"는 해석이 실배포
  분류기의 실제 판단 시점과도 자연스럽게 대응). 중점/다수결 등 대안은 §12.3에서
  대안으로만 언급하고 채택하지 않는다(구현 복잡도 대비 이득 불명확).
- **경계 검증 로직 완화 필요**: `q_abs.__init__`의 `seg_len > narrowest_zone_width`
  거부 조건은 `zone_invasion=True`일 때는 적용하지 않아야 한다(침범이 허용되므로
  좁은 존에서도 세그먼트를 만들 수 있음).

**§11 라벨-독립성 체크리스트 대비 검증**:
- 원인(A) 라벨 누수: 영향 없음 — 시작점/경계 판정 기준 자체는 그대로(q_frac_wide는
  여전히 q_tot 기반, q_abs는 여전히 cap_ref 기반)이고, 단지 "어디까지 내용을
  포함시키느냐"만 바뀐다. 다만 **q_frac_wide는 이미 원인(A) leak이 있으므로
  (§11.3), 그 축에서 zone_invasion을 켜면 침범한 만큼 leak이 있는 콘텐츠가 더
  섞인다는 점**은 유의해야 한다 — 근본 원인은 zone_invasion이 아니라 q_frac_wide
  자체의 기존 결함이다.
- 원인(B): 무관(파라미터 튜닝 방식이 아니라 콘텐츠 포함 규칙이라 test 셀 참조 문제
  자체가 없음).
- 원인(C) 선택 편향: **해결하지 않는다.** 노화 시 존 자체가 소실되는 문제(§11.1-③)
  는 zone_invasion과 별개 — 오히려 침범을 허용해도 그 존에 도달할 데이터 자체가
  없으면(예: 노화된 셀이 high 존 근처에 도달 못함) 여전히 결측된다.

**예상 영향 — 정성적 가설(미실측, 실험 필요)**:

| 방향 | 근거 |
|---|---|
| (+) 학습-배포 분포 격차 축소 가능 | 이전 대화에서 검토한 iMOE/clustered-segments 논문 패턴과 정렬 — 경계를 걸친 콘텐츠를 훈련 중에도 보게 됨 |
| (+) 좁은 존(narrow zone)의 표본 손실 완화 | 지금은 `seg_len > 존폭`이면 아예 세그먼트 생성 불가(q_abs 검증에서 명시적으로 막혀 있음) — 침범 허용 시 그 제약이 풀림 |
| (−) 시나리오 라벨의 "순도" 저하 가능 | 시작점 기준 라벨은 유지되지만 콘텐츠 일부는 다른 존의 것 — 시나리오별 특화(이 프로젝트의 핵심 설계 근거, 이전 대화의 "산업적 의미" 논의 참고)가 희석될 수 있음 |
| (−) 분류기 정확도(`clf_acc`) 하락 가능성 | 경계 근처 세그먼트일수록 내용이 이웃 존과 섞여 분류가 더 어려워짐 — `260731_RESULTS.md`에서 확인한 확산성 confusion 패턴과 유사한 방향으로 chg_lo↔chg_mid 등 인접 클래스 혼동 증가 예상 |
| (±) oracle R²는 방향 불확실 | 순도 저하로 헤드별 학습이 더 이질적인 분포를 봐야 하지만(악화 요인), 동시에 학습 데이터가 실배포와 더 닮아 일반화가 개선될 수도 있음(개선 요인) — 사전 예측이 어려운 영역이라 반드시 실측 필요 |

**진단 계획**: `SegmentRecord.meta`에 `invasion_frac`(세그먼트 Q-스팬 중 자기 존
밖에 있는 비율) 필드를 추가하고, `4_hi_analysis/seg_diagnose.py`의 `collect_stats`
계열에 이 값의 시나리오별 평균/분포를 집계하는 로직을 추가한다 — 기존
`coverage`(random_segment 누락 비율 공개)와 같은 투명성 원칙.

### 12.2 `random_length` — 랜덤 길이 샘플링 (건강 시점 Q의 5~15%)

**목적**: 존 전체가 결국 데이터로 쓰이도록(요청하신 "랜덤한 세그먼트를 시나리오별로
합치면 해당 시나리오의 모든 구간이 데이터로 사용되는 게 목적") 세그먼트 길이·위치를
모두 무작위화하고, 여러 사이클·셀에 걸쳐 집계됐을 때 존의 커버리지가 현재보다
훨씬 촘촘해지게 한다.

**길이 정의 — 반드시 BOL(건강 시점) 기준으로 앵커링**: 요청하신 "건강한 시점의
Q의 5%~15%"는 `q_abs.py`가 이미 쓰고 있는 `_cap_ref`(그 셀·방향의 첫 유효 사이클
`q[-1]`, BOL 고정 기준) 메커니즘을 재사용해야 한다 — **그 사이클 자신의 `q_tot`를
쓰면 안 된다.** 길이가 `q_tot`(≈SOH)의 함수가 되면 §11.1-①과 동일한 원인(A)
라벨 누수가 발생한다(길이 자체가 SOH와 상관된 값이 되어버림). `cap_ref`는 첫
사이클에 고정되고 이후 사이클과 무관하므로, `random_frac ~ Uniform(0.05, 0.15)`를
독립적으로 뽑아 `seg_len = random_frac × cap_ref`로 정의하면 라벨-독립성이
유지된다(§11.1-①이 요구하는 "배포 전 1회 계측 가능한 고정 기준" 원칙과 동일).

**축별 사전 준비 상태(중요 — 구현 순서에 영향)**:

| 축 | `cap_ref`(BOL 앵커) 존재 | `random_length` 적용 난이도 |
|---|---|---|
| `q_abs` | **이미 있음**(`_cap_ref` dict) | 낮음 — 기존 좌표계(정격용량 비율)를 그대로 씀 |
| `vqslope` | 없음(zone 자체가 순간 landmark 기준, cap_ref 불필요했음) | 낮음 — `q_abs`의 `_cap_ref` 캐시 로직을 거의 그대로 이식하면 됨(zone 경계 정의는 안 건드리고 길이만 cap_ref 기준으로 추가) |
| `q_frac_wide` | 없음, **게다가 zone 경계 자체가 이미 `q_tot` 기반(원인 A leak 기존 보유, §11.3)** | 높음 — `random_length`만 cap_ref로 앵커링해도 zone 경계 자체의 기존 leak은 안 고쳐짐. 제대로 하려면 q_frac_wide의 zone 경계 정의 자체를 BOL 앵커링으로 마이그레이션해야 하는데, 이는 이 옵션의 범위를 넘는 더 큰 변경(사실상 q_abs와 유사한 축이 됨) |

→ **구현 우선순위 제안**: `q_abs` 먼저(가장 자연스러운 적합), `vqslope`는 `cap_ref`
캐시만 이식하면 되므로 다음 순서, `q_frac_wide`는 이번 범위에서 제외하고 별도
과제(zone 경계 BOL 마이그레이션)로 분리 — 억지로 넣으면 "길이는 안 새는데 경계는
새는" 어중간한 상태가 되어 리뷰어에게 설명하기 더 어려워진다.

**n_samples 자동 배정 — `q_abs.adaptive_samples` 공식의 일반화**: 길이가
고정값이 아니므로 기존 `_compute_adaptive_plan`의 `n_theta = 1 + r_k /
((1-max_overlap) × seg_len)` 공식에서 `seg_len` 자리에 **기대 길이**
`E[seg_len] = (0.05+0.15)/2 × cap_ref = 0.10 × cap_ref`를 대입해 목표 개수를
근사한다. 실제 draw마다 길이가 다르므로 이 n_k는 "기대 커버리지 목표"이지
q_abs의 기존 결정론적 보장(고정 seg_len이므로 겹침률이 정확히 계산됨)과 달리
분산이 있다는 점을 설계 문서·코드 주석에 명시해야 한다(과장 금지).

**클리핑 금지 원칙 재확인(§11.2-⑤)**: 뽑힌 `random_frac × cap_ref` 길이가 그
존의 남은 폭보다 크면(특히 노화 사이클에서 존이 좁아지는 경우), **길이를 존
폭에 맞춰 줄이면 안 된다** — 그러면 길이가 다시 그 사이클 상태의 함수가 되어
원인(A)가 재발한다. `random_segment`의 기존 원칙과 동일하게, 그 시도는
스킵하거나(존 전체 1개 fallback은 이 경우 의미가 약함 — 길이 자체가 랜덤이므로
그냥 실패로 카운트하고 다음 시도로 넘어가는 게 자연스러움), 또는 `zone_invasion`
과 함께 켜져 있으면 다음 존으로 침범을 허용해 온전한 길이를 확보한다.

**`zone_invasion`과의 조합 시맨틱스**:

| `zone_invasion` | `random_length` | 동작 |
|:---:|:---:|---|
| off | off | 기존 그대로(고정격자 또는 기존 `random_segment`) |
| off | on | 길이는 랜덤(5~15%×cap_ref)이지만 **존 경계를 못 넘음** — 길이가 존 잔여폭보다 크면 그 시도는 스킵(재시도 또는 카운트만 하고 넘어감), 존 폭에 맞춘 클리핑은 금지(위 원칙) |
| on | off | 길이는 고정(기존 `seg_len`/`seg_len_pts`)이지만 시작점 근처면 존을 넘어갈 수 있음(§12.1) |
| on | on | 길이·시작점 모두 랜덤이고 존 경계 제약도 없음 — "존은 라벨링 기준일 뿐, 실제 관측 윈도우는 존과 무관하게 뽑는다"는 이전 대화의 원안에 가장 가까운 모드 |

**예상 영향 — 정성적 가설**:

| 방향 | 근거 |
|---|---|
| (+) 존 전체 커버리지 대폭 개선 | 지금은 `n_samples`개 고정/제한 위치만 샘플링 — 랜덤 길이+위치를 많은 사이클·셀에 걸쳐 누적하면 존의 거의 전 구간이 결국 콘텐츠로 등장(§12.0 동기 2번 해소) |
| (+) HI-SOH 상관의 강건성 검증 강화 | 다양한 길이·위치에서 반복 관측되므로, 특정 길이·위치에만 우연히 맞는 얕은 상관(과적합)이 아니라는 걸 더 신뢰성 있게 주장 가능 |
| (−) HI 계산에 새 잡음원 추가 | 길이 자체가 라벨과 무관하더라도, 길이의 **분산**이 통계 기반 HI(분산·표준편차 계열)의 값에 영향을 줄 수 있어 모델이 이 잡음을 새로 marginalize해야 함 — 순수 정보량은 늘지만 학습 난이도도 같이 오를 수 있음 |
| (−) 존 폭이 좁은 시나리오(narrow zone)에서 스킵률 증가 | 5~15% 길이 중 큰 값(15%)이 뽑히면 좁은 존에서 실패율이 올라감 — `zone_invasion=off`와 조합하면 특히 두드러질 것으로 예상 |
| (±) 원시 곡선(raw CNN/flatten) 융합과의 상호작용 불확실 | `_resample_segment`(q_frac 정규화 [0,1] 그리드 보간)는 길이 무관하게 이미 잘 동작하므로 호환성 문제는 없으나, 이번 세션에서 raw 융합 자체가 조합별로 효과가 엇갈렸던 것(`docs/260801_RESULTS.md`)을 고려하면 random_length가 raw 융합과 결합했을 때의 효과는 별도로 재검증해야 함 |

**진단 계획**: `random_segment`의 기존 `coverage`(covered/total 포인트 비율) 집계
메커니즘을 그대로 재사용 — 이미 "샘플링 안 된 포인트 비율"을 시나리오별로
공개하는 인프라가 있으므로 그걸 그대로 쓰면 된다. 추가로 `candidate_n_points`에
해당하는 **실제 draw된 길이 분포**(Q-fraction 단위)를 별도 기록해 히스토그램으로
공개한다(현재 point-count 기준 분포 기록과 대응).

### 12.3 검토했으나 채택하지 않은 대안

- **경계 침범 세그먼트의 라벨을 "다수결"/"중점 존"으로 정하기**: §12.1에서
  "시작점 존" 규칙 대신 검토했으나, (a) 구현이 복잡해지고(포인트별 존 소속을
  다시 계산해야 함), (b) 실배포 분류기가 실시간으로 판단하는 시점(관측 시작
  시점)과의 대응성이 시작점 규칙보다 떨어진다고 판단해 기각. 실측 후 시작점
  규칙의 라우팅 정확도가 유의미하게 나쁘면 재검토 후보로 남긴다.
- **`random_length`의 길이 분포를 5~15% 균등이 아니라 존 폭에 비례**: 존마다
  최적 길이가 다를 수 있다는 논리는 합리적이나, "존 폭에 맞춰 뭔가를 조정"하는
  방식은 §11.2-⑥에서 이미 한 번 시도했다 폐기한 접근(EOL 잔존용량 clip)과 같은
  함정에 빠질 위험이 있다 — 존 폭 자체가 노화 무관 고정값(zone 경계는 static)이라
  이번엔 라벨 누수 위험은 없지만, "왜 존마다 다른 분포를 썼는가"를 사후에 정당화하기
  어려워 리뷰어 설득력이 떨어진다고 판단해 고정 5~15%(요청하신 스펙 그대로) 채택.

### 12.4 구현 순서 제안 (미착수 — 사용자 확인 후 진행)

1. `q_abs`에 `random_length`만 우선 구현(cap_ref 이미 존재, 가장 적은 변경).
   `_compute_adaptive_plan`을 기대 길이 기반으로 일반화하는 부분이 핵심 변경점.
2. `q_abs`에 `zone_invasion` 구현 — `_start_positions` 상한 완화 + 생성자 검증
   조건 완화. `random_segment` 모드는 `sample_random_windows` 시그니처 확장 필요.
3. `_random_seg.py`의 `sample_random_windows`를 "시작점 풀 / 콘텐츠 풀 분리"로
   확장(2번에서 필요) — `q_frac_wide`/`vqslope`의 기존 `random_segment` 호출부는
   하위호환 유지(풀을 분리 안 하면 기존과 동일 동작).
4. `vqslope`에 `_cap_ref` 캐시 이식 후 `random_length`/`zone_invasion` 적용.
5. `q_frac_wide`는 이번 범위에서 보류(§12.2 표 참고) — 필요해지면 별도 설계로.
6. `seg_diagnose.py`에 `invasion_frac`/길이 분포 진단 추가, 실 데이터로 정적
   진단(학습 없이 세그먼트 통계만) 먼저 실행 — 기존 세션에서 확립한 패턴(값비싼
   학습 전에 `seg_diagnose`로 생존율·커버리지부터 확인)을 그대로 따른다.
7. 정적 진단 결과가 합리적이면(극단적 스킵률·커버리지 붕괴 없음) 그때 실제
   학습(스모크 → 본 실험) 진행.
