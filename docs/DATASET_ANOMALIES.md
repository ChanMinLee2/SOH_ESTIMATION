...
위 "충전 | MIT | ~644건 | ~43셀"이 셀 전체에 고르게 퍼진 게 아니라 **거의 전부 batch2에
쏠려 있다**는 게 `3_integrity/charge_gap_census.py` 전수조사(`_4_data_hi/clean/` 기준
MIT 123셀 + HUST 77셀, `chg_gap_seg`가 이미 `True`로 찍힌 충전 phase 행 비율을 셀·배치별
집계, 결과는 `3_integrity/outputs/charge_gap_census.csv`)로 확인됐다.

| 데이터셋 | 배치 | 셀 수 | 충전 행 중 `chg_gap_seg=True` 비율(평균) | 범위 | 사이클 100% 단절된 셀 수 |
|---------|------|------|------------------------------------------|------|--------------------------|
| MIT | b1 | 40 | 2.2% | 0%~23.2% (b1c44, b1c45 예외적으로 높음) | 0 |
| MIT | **b2** | **43** | **99.9%** | **99.2%~100%** | **37 / 43 (나머지 6셀도 사이클의 99%+ 단절)** |
| MIT | b3 | 40 | 0% | 0% | 0 |
| HUST | 전 그룹(1-~10-) | 77 | 0% | 0% | 0 |

**해석**: batch2는 셀 하나가 예외적으로 이상한 게 아니라 **배치 전체가 충전 phase에서
시간 단절 판정을 사실상 항상 트리거**한다 — batch1/batch3와 다른 충전 로깅 방식(샘플링
간격 또는 프로토콜 차이)이 원인으로 추정된다(정확한 원인은 미확인 — Severson 원본
메타데이터 확인 필요). 실질적 영향:
- Step2 필터4 로직(충전 단절 → 충전 phase 행만 제거, 방전은 보존)에 따라 **batch2 셀의
  충전 phase 데이터는 정리된 데이터(`_4_data_hi/clean/`)에 사실상 존재하지 않는다** —
  방전 HI/피처는 영향 없음.
- `validate_capacity_error.py`의 대표 셀 곡선 플랏에서 batch2 대표 셀(b2c0)은 충전
  전류적산 점선이 항상 비어 보인다 — 스크립트 버그가 아니라 이 배치 특성 때문.
- q_frac_ref 등 향후 충전 phase 기반 피처/세그먼트를 설계할 때 MIT batch2는 표본에서
  사실상 빠진다는 점을 감안해야 한다(§6 Phase 2 "데이터셋별 확인 사항"에 참고).
...
| 충전 구간 내 시간 단절 | **batch2 43셀 전부**(사이클의 99%+) — 충전 phase만 제거, 방전은 보존. batch1/batch3는 거의 0% (상세: Step 2 — 필터4 "충전 단절의 배치별 편중" 참조) |
