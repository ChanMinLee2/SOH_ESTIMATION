...
model_lib/training/scr_trainer.py, model_lib/models/hard_concrete.py,
model_lib/utils/gate_io.py(게이트 JSON 저장 함수 — 2026-09-24까지는
model_lib/legacy/train_scr.py였음, Stage0 삭제 때 옮김)는 단 한 줄도 수정하지
않는다 — 전부 그대로 IMPORT해서 재사용(SCRModel/SCRLoss/build_datasets/
L0LambdaScheduler)하고, "에폭을 몇 번 돌고 어느 시점을 최종본으로 저장할지"를
결정하는 학습 루프만 이 파일이 독자적으로 새로 짠다.
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--max-epochs", type=int, default=P.ACTIVE_MAX_EPOCHS,
                   help="cfg의 training.epochs(기본 500) 대신 쓸 상한. 시간 단축용 — "
                        "--patience 조기종료가 그 전에 걸리면 이 값까지 안 감")
    p.add_argument("--patience", type=int,
                   default=P.ACTIVE_PATIENCE if P.ACTIVE_PATIENCE is not None else 60,
                   help="L0 완전 램프 이후, 게이트 포화도가 이 에폭 수만큼 연속 개선 없으면 "
                        "조기 종료(parameters.py 기본 60 — main_qfref_S_p60.yaml과 동일 근거). "
                        "베스트 체크포인트 자체는 patience 길이와 무관하게 항상 보존되므로 "
                        "결과에는 영향 없고 학습 시간만 줄어듦. 0이면 조기종료 비활성(항상 --max-epochs까지)")
    p.add_argument("--batch-size", type=int, default=P.ACTIVE_BATCH_SIZE,
    ...
    p.add_argument("--combined-redundancy-json", default=P.ACTIVE_COMBINED_REDUNDANCY_JSON,
                   ...
                   help="kernel.py의 3차(결합 raw+kernel, 시나리오별) 다중공선성 "
                        ...
                        "선행 단계라 raw 전용으로만 쓰는 경우는 아직 미지원). parameters.py 기본 None.")
    p.add_argument("--tag", default=P.ACTIVE_P1_TAG)
    ...
    p.add_argument("--lambda-l0-override", type=float, default=P.ACTIVE_LAMBDA_L0_OVERRIDE,
                   ...
                   help="loss.lambda_l0을 이 값으로 강제 고정(lambda_l0_auto/yaml 값 무시, 최우선순위). "
                        "2026-09-21부터 parameters.py 기본값(0.000237)이 항상 활성 — None으로 끄면 "
    ...
    p.add_argument("--interaction-json", default=P.ACTIVE_INTERACTION_JSON, dest="interaction_json",
                   help="interaction.py 산출물 경로(v4). significant=True인 "
                        "HI만 기존 scen_gates(시나리오별)로 남기고, 나머지는 새 shared_gate "
                        "1개로 통합한다. parameters.py 기본 None.")
    p.add_argument("--l0-warmup-epochs-override", type=int,
                   default=P.FIXED_L0_WARMUP_EPOCHS_OVERRIDE, dest="l0_warmup_epochs_override",
                   help="loss.lambda_l0_warmup_epochs을 이 값으로 강제 고정(yaml 값 무시). "
                        "warmup epoch 스위핑 실험(docs/260825_RESULTS.md) 전용 — parameters.py "
                        "기본 None이면 기존 동작(yaml 값, 기본 50)과 동일. training.warmup_epochs"
                        "(learning rate warmup, 별개 값)에는 영향 없음.")
    p.add_argument("--l0-norm-constant", type=int, default=P.FIXED_L0_NORM_CONSTANT,
                   dest="l0_norm_constant",
                   help="docs/260915_RESULTS.md — scr_loss.py의 _l0_penalty가 시나리오별 "
                        ...
                        "원래 2로 나눔)과 동일한 정규화 강도로 맞출 수 있음. parameters.py 기본 "
                        "None이면 기존과 동일 동작.")
    p.add_argument("--hi-cost-weighted-l0", action="store_true", dest="hi_cost_weighted_l0",
                   ...
                   help="2026-09-19: L0 페널티에 HI 카테고리별 계산비용(stat/diff/lfp/morph, "
                        "utils/hi_schema.py::CATEGORY_COSTS)을 가중치로 곱한다. 꺼져 있으면"
                        "(parameters.py 기본) raw/커널 HI 전부 균일 비용 1.0 — 순수 '활성 게이트 "
                        "개수'만 페널티가 된다.")
    p.add_argument("--val-rmse-epsilon", type=float, default=P.FIXED_VAL_RMSE_EPSILON,
                   dest="val_rmse_epsilon",
                   help="2026-09-18(체크포인트 선택 기준 변경): val_rmse가 best보다 이 값 "
                        ...
                        "낮은 epoch을 대신 채택한다. parameters.py 기본값 0.0005는 "
...
def main() -> None:
    ...
    # 중단해도 되는지 확인하다가 9_eval/test.py가 필수로 요구하는 run_dir/config.yaml이
    # 아직 없어서 best_by_saturation.pt가 이미 저장돼 있어도 테스트가 즉시
    # FileNotFoundError로 막히는 문제가 있었다. 여기서 미리 써두면
